...
export async function ensureSessionKey(force = false, retries = 3): Promise<string> {
  ...
  if (existingKey && !force) {
    try {
      const validationUrl = new URL("/api/register", window.location.origin).toString()
      const response = await fetch(validationUrl, {
        headers: {
          "X-Session-Key": existingKey,
          Accept: "application/json",
        },
      })

      // If session is valid (backend returns 200), use it
      if (response.ok) {
        return existingKey
      }
      ...
      log("Session validation failed, will try to use existing key", { error })
      return existingKey
    }
  }
